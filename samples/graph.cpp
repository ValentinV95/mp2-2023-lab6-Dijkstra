...
void WeightedGraph::add_edge(size_t departure, size_t destination, size_t weight)
{
	...
	if (departure != destination && departure < vertices_num && destination < vertices_num && weight < INF)
	{
...
{
	if (start_vertex >= graph.get_vertices_num())
	{
...
{
	if (destination_vertex >= vertices_number || destination_vertex == start_vertex)
	{
